...
def run_2D_dist(ref, test, output):
    ...
    test_df['mol'] = df['truncate_SMILES'].apply(Chem.MolFromSmiles)
    test_df['fp'] = df['mol'].apply(lambda mol: AllChem.GetMorganFingerprintAsBitVect(mol, radius=3, nBits=2048))
    ...
    np.save(output + '.npy', distance_matrix)
